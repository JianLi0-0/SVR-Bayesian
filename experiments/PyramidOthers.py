...
def PowellRun(dataSet, serie, image, transformation, numberOfLevels):
...
def BFGSRun(dataSet, serie, image, transformation, numberOfLevels):
    volume = dataSet.getVolume()   
    goalSlice = dataSet.getGoalSlice(serie, image)
    pyramid3D = Pyramid(volume.GetDimension(),Config.CONTINUOUS)
    pyramid2D = Pyramid(goalSlice.GetDimension(),Config.CONTINUOUS)
...
def LBFGSRun(dataSet, serie, image, transformation, numberOfLevels):
    volume = dataSet.getVolume()   
    goalSlice = dataSet.getGoalSlice(serie, image)
    pyramid3D = Pyramid(volume.GetDimension(),Config.CONTINUOUS)
    pyramid2D = Pyramid(goalSlice.GetDimension(),Config.CONTINUOUS)
